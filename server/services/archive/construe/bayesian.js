...
var Bayesian = function (options) {
  ...
  case 'redis':
    this.backend = new (require('./backends/redis')).RedisBackend(
				backend.options
			);
    break;
  case 'localstorage':
    this.backend = new (require('./backends/localStorage')).LocalStorageBackend(
				backend.options
			);
    break;
  default:
...
Bayesian.prototype = {
  ...
  getProbsSync: function (doc) {
    ...
    console.log(words, cats, counts)
    return this.getCatProbs(cats, words, counts);
  },

  bestMatch: function (probs) {
    var max = _(probs).reduce(
		function (max, prob, cat) {
